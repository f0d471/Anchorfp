...
只在 else 分支出现的信号，复位期间保持不变，而「保持不变」正是时钟使能的语义。
综合器会把 rst_n 接成这些寄存器的 CE，于是复位网扇出到成百上千个 CE 端，
形成一条零级逻辑、纯布线的高扇出路径。修法是把控制位与数据位拆成两个 always，
数据位无条件更新。

用法：python3 scan_reset_as_ce.py [rtl 根目录，默认 rtl]

报出来的不一定都要改：数据位真的需要复位值时，接复位是对的。本脚本只负责把
「只有个别信号有复位值、其余十几个没有」的地方找出来，是否要改由人判断。
阈值定得保守，宁可漏报也不刷屏。
"""
