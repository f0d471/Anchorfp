/* gen_recip_lut.c —— 生成 fp32_recip 的牛顿迭代初值表（[#37] §15.18）
 *
 * 这个文件的存在本身就是一条教训：原来的 recip_lut.mem 是**没有生成器**的，
 *    表怎么来的、按什么口径存的，只能从数值反推（我上一轮就是逐条反推出
 *    "存的是区间左端点的 2/m"）。凡是进 RTL 的表，都必须有可复现的生成器。
 *
 * 表的内容
