...
#   用法：vivado -mode batch -source ooc_mac.tcl -tclargs <标签> [WinUp WinFrac UseCarrySave FuseMul 源目录 GrowW]
#   缺省参数 8 8 0 1 ../rtl 15；UseCarrySave 只能传 0，非零档不在支持集合内
#   WinFrac 扫的是精度维（认证精度位数 Q），GrowW 扫的是容量维（项数上限 2^GrowW）
...
synth_design -top fp32_mac_unit -part xc7a200tfbg676-1 -mode out_of_context \
    -generic WinUp=$up -generic WinFrac=$fr -generic UseCarrySave=$cs -generic FuseMul=$fuse -generic GrowW=$grow
