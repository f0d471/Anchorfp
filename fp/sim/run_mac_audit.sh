...
# 三 C5 的项数边界。上限为 2^GrowW，非零 psum 也算一项，K 项加 psum 等于 K+1 项。
#    满额的合法 tile 不许报，超一项必须报；三个 tile 连跑，抓「新 tile 沿用旧计数」的误报。
#    GrowW 从 RTL 现取，取不到即判失败
MAC_GROWW=$(sed -n 's/.*parameter *GrowW *= *\([0-9]\+\).*/\1/p' "$FP/fp32_mac_unit.v" | head -1)
if [ -z "$MAC_GROWW" ]; then
    echo "  FAIL 取不到 fp32_mac_unit.v 的 GrowW，C5 边界无法确定"
    BAD=$((BAD+1)); MAC_GROWW=0
fi
MAC_MAXTERM=$((1 << MAC_GROWW))
echo "==== C5 项数边界：满额不误报，超限必报（GrowW=$MAC_GROWW，上限 $MAC_MAXTERM 项）===="
...
c5_case $((MAC_MAXTERM - 1)) zero    "满额 tile"
c5_case $MAC_MAXTERM         nonzero "超一项"
