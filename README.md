...
[![scalar](https://img.shields.io/badge/scalar%20add%2Fmul-0%20ULP-2ea043)](#41-精度)
...
七个可独立例化的 Verilog-2001 模块，加一份写明它们各自算什么、算到什么精度的契约。
标量六条在 FTZ 语义下逐位等于 IEEE-754，45 万条随机向量 0 ULP；
点积累加器用一把 88 位定点尺子，一块之内不舍入。

两条线的用处不同。标量线补的是「核里没有 FPU，一次浮点乘法要几十上百拍」这个洞；
点积线补的是「矩阵乘、卷积、投影底下都是长点积，而通用 FPU 每算一项舍一次、
误差随长度累积」这个洞。两条线共用同一份数值契约，也共用同一批乘法与加法单元。

![architecture](docs/figures/fig-arch.svg)

导航：[定位](#1-定位) · [数值契约](#2-数值契约) · [架构与延迟](#3-架构与延迟) ·
[结果](#4-结果) · [工程取舍](#5-工程取舍) · [验证方法](#6-验证方法) ·
...
做给 FPGA 上的边缘加速器：控制核常常没有 FPU，热点既有标量浮点也有长点积，
...
五个计算单元互相独立，可以单独例化，也可以不要 `fp32_fpu_top` 这一层。
`fp32_mac_unit` 与它们并列，不经路由层。

**流水切得很浅是有意的。** 目标场景里调用方多半会等结果，
延迟直接就是这条运算的价格；加法器多切一级，每一次浮点加法就多付一拍。
所以准则是「每个单元的延迟压到能跑出频率的最小值」，
而不是常见的「切深流水换频率」。要更高吞吐，需要的是把多个单元并起来，
不是把单个单元切深。
...
| Anchorfp | 88 位定点窗口，基准动态 | 每次累加一次 | 1278 LUT / 496 FF，WNS 7.53 ns |
| 精确累加（Kulisch / posit quire） | 555 位，覆盖全指数范围 | 全程不舍入 | 9106 LUT / 1956 FF，WNS 0.61 ns |

右边两列是同一份 RTL、同一个综合流程、同一颗器件（xc7a200t-1，20 ns 约束，
OOC 单 lane）量出来的：把 `GrowW` 从 15 调到 482 就得到覆盖 FTZ 全域的 555 位窗口。
面积是 7.1 倍，但更值得看的是时序——逻辑级数从 17 涨到 49，20 ns 的余量从
7.53 ns 掉到 0.61 ns。精确累加不只是费面积，它同时把频率吃掉。

555 位这个数是算出来的：FTZ 语义下最小乘积是 2⁻²⁵²，它的最低有效位落在 2⁻²⁹⁹，
最大乘积不到 2²⁵⁶，一位不丢就得把这两端之间全排上。88 位是它的 16%。

88 也是算出来的，不是试出来的。累加器要装下的是三样东西，各自有独立的来源：

```
 48   两个 binary32 尾数相乘的未舍入积，24 × 24
 16   WinG，基准量子化之后最大项落点的区间宽度
  8   WinFrac，最大项最低位之下留的位，决定认证精度 Q
 15   GrowW = ⌈log₂N⌉，一次累加最多 32768 项的求和增长
  1   符号
 ───
 88
```

反过来读这张表就是这把尺子的规格：给定项数上限 `N` 与目标精度 `Q`，
宽度由 `AccW = 48 + WinG + WinFrac + ⌈log₂N⌉ + 1` 与 `Q = 46 + WinFrac − ⌈log₂N⌉`
两条式子定死，没有可调的余地。只要 255 项的使用者把 `GrowW` 传成 8，
同样的 `Q = 46` 只需要 81 位。88 位对应的是 32768 项这一档。

式子里没有 `WinUp`。不变式 `B ≥ E + WinUp` 保证任何单项对齐后的最高位都不超过
`TW−1−WinUp`，窗口顶上那 `WinUp` 位不是项的落点，本来就是求和增长的空间——
这一点此前被算漏过一次，把项数上限说成了 255，见 `docs/reports/10` 第 3.3 节。
...
按这把尺子，承诺切成三档，顺序就是优先级：

- **A 档不可协商**，违反即缺陷，不计面积：定义域内正确舍入；同一运算在不同调用路径上
  结果相同；比较器给出真实序关系；特殊值不产生垃圾；能在编译期查的必须在编译期查；
  **说明书里每一个精度数字都要有验过的出处**。
- **B 档明示偏离**，写进契约、使用者可预期：冲零规则、下溢判定的时点、只有一种舍入模式、
  不产生异常标志、近似原语各自的误差预算、没有融合乘加、窗口累加的那条不等式。
- **C 档性能面积功耗**，排在前两档之后。

完整条目在 [`rtl/datapath-manual.md`](rtl/datapath-manual.md) 第 11 节。
...
「只有对角线匹配」这类假象。NaN 的方向按 IEEE 的无序语义，
...
`N` 是一次累加的项数上限，`WinFrac` 是例化参数。契约给的是这条公式而不是一个数：
默认 `WinFrac = 8` 时，`N ≤ 255` 得 `Q = 46`，`N ≤ 4096` 得 `Q = 42`，
`N ≤ 32768` 得 `Q = 39`。项数越多界越松，这是求和增长本身的代价，与实现无关。
...
刚好把第 4.4 节那四条回收省下的 195 个 LUT 花光；而且一个布尔粘滞位恢复不了被丢残差的符号，
正负项抵消时它可能把结果推向错误的方向。花掉全部面积收益，换来的还不是正确性。
真要那一个 ULP，正解是把尺子加长到覆盖整个指数范围，那是另一个设计点。

第 5 条不是这个实现特有的：任何基准跟着最大值走、长度又有限的累加器都有这一条。
除了写进契约，模块还引出一位 `mac_prec`，在基准重锚（旧和被整个丢弃）时逐块置位，
供调用方汇总。它只报这一类，不报普通对齐丢位——**一个假阳性率高到恒为真的信号
没有信息量**，所以它的核心判据是反面的那条：普通抬基准时必须不置位。
...
op 编码在 `rtl/fp32_ops.vh`，只有域宽和六条运算的编号。
接别的核改这一个头文件即可，五个计算单元本身与编码无关。
...
最后一行是融合乘加单独的收益。同样的窗口，乘积先舍一次，误差就回到 1027。

### 4.2 每次运算的周期数

![latency](docs/figures/fig-latency.svg)

图里「软件」一列是没有 FPU 的核用整数指令模拟一次浮点运算的周期数，
「硬件」一列是这套数据通路的实测单价，含调用方等待结果的握手开销，
所以 3 拍的加法器算成 5.99 拍。
...
三维高斯溅射渲染器的几何级，把空间中的高斯点投影成屏幕上的椭圆，全是浮点。
逐个工作点实测，每一步只加一种运算，从 10559 ms 累计降到 1132 ms。
这条曲线整条都是标量线的战绩，点积累加器不在这条路径上。

它停在 1132 ms，是因为再往下的账已经不属于浮点：重新拆账之后，循环控制、寻址、
访存、整数分支合计占 47%，是最大单项（乘法 18.5%）的 2.5 倍，
而乘法的 4.99 拍已经是硬件价。
...
最后一条值得单说，因为它是个容易复发的写法问题。数据寄存器写在复位块的 `else` 分支里，
复位期间保持不变，而保持不变正是时钟使能的语义，于是综合器把 `rst_n` 接成了这一整组
寄存器的 CE。88 位乘 32 条 lane，几千个使能端挂在一根网上，
布线后的最差路径变成零级逻辑、纯布线、终点是 CE 脚。

```
   slack = 0.053   levels = 0
   from  <复位同步器>/delay_reg[1]/C
   to    <lane 18>/u_mac/n1_mag_reg[11]/CE
```

单 lane 离线综合量不出来（1275 对 1277，在噪声里），因为 88 个使能端算不上高扇出，
要例化多份才看得见。`syn/scan_reset_as_ce.py` 能把这个写法从整个 rtl 目录里扫出来，
本仓改完之后已经不在它的输出里。
...
两层原来共用同一份位型解释与乘积构造代码——**共用输入侧就等于只有一层**，
那份代码若有错，两层会一起错。拆开各写一份之后，把第一层的乘积阶码改坏一位，
两层对拍从 maxULP 0 变成 139809444，这个数才是「两层真的独立了」的证据。
...
本仓做的：把这些件按「延迟即单价」这个约束重新配平，延迟优先、单价实测、契约写死；
...
| | 章 | 覆盖内容 |
|---|---|---|
| 01 | [标量浮点协处理器接口设计](docs/reports/01-标量浮点协处理器接口设计.md) | 接入方式、延迟约束、定位与三档契约、握手与分发 |
